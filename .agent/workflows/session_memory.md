...
- **Tính năng mới bổ sung**:
  ...
  - Hỗ trợ chọn Doanh nghiệp tùy chỉnh và Vị trí ứng tuyển riêng biệt: Thiết kế lại form thiết lập trong `StartInterview.jsx` bổ sung nút chuyển đổi (Toggle) "Tôi muốn tự nhập tên doanh nghiệp khác" và trường nhập liệu vị trí ứng tuyển. Vị trí ứng tuyển giờ đây hỗ trợ chọn trực tiếp từ danh sách các vị trí đang tuyển của công ty đó (được trích xuất động từ database) thông qua Select Dropdown, hoặc cho phép bật Toggle để tự gõ vị trí tùy chỉnh theo ý muốn. Cập nhật API `/api/sessions/start` để nhận diện các tham số tùy biến này, tự động đồng bộ doanh nghiệp mới vào database và kích hoạt Gemini AI sinh 10 câu hỏi phỏng vấn chuẩn xác theo đúng công ty và vị trí ứng tuyển mong muốn của người dùng.
  ...
  - Bổ sung các chính sách RLS SELECT, UPDATE và DELETE công khai: Cập nhật `supabase/schema.sql` thêm chính sách `SELECT` công khai cho `profiles`, chính sách `SELECT` và `DELETE` cho `cv_vault`, chính sách `SELECT`, `UPDATE` công khai cho `interview_sessions`. Việc này giải quyết triệt để lỗi RLS khi Backend gọi truy vấn chèn bản ghi kèm lệnh `.select()` (RETURNING * trong SQL), khi kiểm tra/cập nhật thông tin phiên phỏng vấn, hoặc khi thực hiện xóa tệp tin CV bằng Anon Key của Supabase mà không mang theo Token JWT của ứng viên.
  - Tối ưu hóa UI/UX chọn và tải lên CV: Cải tiến khu vực thiết lập CV tại `StartInterview.jsx`. Nếu người dùng đã có sẵn CV đã lưu trong hồ sơ, giao diện sẽ mặc định ẩn đi khung tải lên cồng kềnh và tự động chọn CV đầu tiên trong hồ sơ. Một nút Toggle "Tôi muốn tải lên CV mới" được cung cấp để người dùng chủ động chuyển sang giao diện upload khi có nhu cầu tải file mới. Sau khi tải lên thành công, giao diện sẽ tự động chuyển về dạng chọn CV và chọn sẵn file vừa tải lên.
  - Tích hợp bộ kiểm tra đồng bộ Profile ứng viên: Thêm logic kiểm tra sự tồn tại của dòng Candidate Profile tương ứng với `candidateId` trong bảng `profiles` tại API `/api/sessions/start`. Nếu profile bị thiếu do đăng ký trước khi tạo trigger, hệ thống sẽ tự động gửi thông tin email ứng viên lên và tự động tạo Profile mới công khai thay thế trên database để ứng dụng tự động khắc phục lỗi cho người dùng mà không cần copy SQL chạy thủ công.
