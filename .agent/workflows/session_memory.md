...
- **Tính năng mới bổ sung**:
  ...
  - Xây dựng hệ thống Blog và Bình luận tương tác hoàn chỉnh: Nâng cấp tệp `Blog.jsx` để hiển thị 6 bài viết chuyên sâu về kỹ thuật (React 19, System Design, NIST Security) và kỹ năng (CV, Salary Negotiation). Hỗ trợ đọc chi tiết bài viết, tự động nhận diện tài khoản người dùng đăng nhập để bình luận hoặc cho phép nhập tên khách độc lập, đồng thời lưu trữ dữ liệu bình luận lâu dài qua `localStorage` (dự phòng theo cấu trúc `x_blog_comments_`).
