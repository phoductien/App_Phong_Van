...
- **Tính năng mới bổ sung**:
  ...
  - Bổ sung các chính sách RLS SELECT và UPDATE công khai: Cập nhật `supabase/schema.sql` thêm chính sách `SELECT` công khai cho `cv_vault` và chính sách `SELECT`, `UPDATE` công khai cho `interview_sessions`. Việc này giải quyết triệt để lỗi RLS khi Backend gọi truy vấn chèn bản ghi kèm lệnh `.select()` (RETURNING * trong SQL) hoặc khi cập nhật lịch sử chat của phiên phỏng vấn bằng Anon Key của Supabase mà không mang theo Token JWT của ứng viên.
